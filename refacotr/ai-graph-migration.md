...
| `ontology.generate_mapping` | `application/mapping/usecase/PreviewMappingUseCase` + `MappingLlmGenerationSupport.generate` | 완료 | LLM 기반 매핑 생성이 `mapping` 도메인으로 이동했고, preview는 레거시처럼 LLM을 강제 사용합니다. 프롬프트도 구 `ontology/service.py`의 규칙과 예시를 반영하며, rootless relation, `_ext_` 확장 속성, 관계 속성 분리 규칙을 그대로 유지합니다. |
