...
- `PESSIMISTIC_WRITE`가 필요하면 Repository에서 `@Lock(LockModeType.PESSIMISTIC_WRITE)`로 선언하라.
- 락 조회 메서드명은 `findBy...ForUpdate` 형태를 사용하라.
- 락 메서드는 일반 조회 메서드와 분리하라. 같은 의미의 조회를 boolean 파라미터로 합치지 마라.
- 가능하면 PK 또는 Unique Key 조건으로 Aggregate Root 단건만 잠가라.
- 락 메서드 반환 타입도 엔티티 중심 계약을 유지해 `Optional<Entity>`를 기본으로 하라.
- 락 대기 시간이 중요하면 `@QueryHints`로 timeout을 명시하라.
- 락 메서드 안에 비즈니스 정책이나 후처리 로직을 넣지 마라.
- 일반 상태 변경 플로우를 JPQL bulk update로 우회하지 마라. 엔티티 로드 후 상태 전이 메서드를 호출하는 흐름을 우선하라.
...
5. 락이 필요하면 `findBy...ForUpdate`와 `@Lock(LockModeType.PESSIMISTIC_WRITE)` 조합으로 선언하라.
6. 저장소 코드에 정책 로직이 섞이지 않았는지 점검하라.
...
- `PESSIMISTIC_WRITE` 사용 시 락 메서드가 `findBy...ForUpdate` 형태로 분리돼 있는가?
- 락 범위가 Aggregate Root 단건 기준으로 최소화돼 있는가?
