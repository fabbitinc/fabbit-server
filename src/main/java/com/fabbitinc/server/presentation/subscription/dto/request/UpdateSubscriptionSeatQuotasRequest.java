...
public record UpdateSubscriptionSeatQuotasRequest(
        ...
        @NotEmpty(message = "seat_quotas는 최소 1개 이상이어야 합니다") List<@Valid SeatQuantityRequest> seatQuotas
) {
    public record SeatQuantityRequest(
            ...
            @NotNull(message = "seat_type은 필수입니다") SeatType seatType,
            @Schema(description = "구매 좌석 수량", example = "3")
            @Min(value = 0, message = "purchased_quantity는 0 이상이어야 합니다") int purchasedQuantity
    ) {
    }
}
