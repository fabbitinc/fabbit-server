...
| -------------- | ------------------- | ------------------------ | -------------------------------------------------- | ----------------------------------- |
| **MVP**        | QCAD Pro Server CLI | Mayo (MayoConv AppImage) | €478 (QCAD, 영구/서버당) + 무료 (Mayo)             | 모놀리스 (Spring Boot + Docker 1대) |
| **Growth**     | ODA Sustaining SDK  | ODA Sustaining SDK       | $7,500 첫해 / $4,500 연간 (회사 단위, 서버 무제한) | API + 변환 워커 분리                |
| **Enterprise** | ODA SDK             | ODA + CAD Exchanger SDK  | +$15K~50K/년 (견적 기반, 매출 연동 티어)           | 동일                                |

---

## 2D 도면 포맷 (많이 쓰는 순)

| 포맷    | 설명                                         | MVP (QCAD)            | Growth (ODA)                             | Enterprise | 변환 타겟           |
| ------- | -------------------------------------------- | --------------------- | ---------------------------------------- | ---------- | ------------------- |
| **DWG** | AutoCAD 네이티브. 한국 제조업 2D 사실상 표준 | ✅ QCAD `dwg2pdf`     | ✅ ODA SDK (심층 파싱, 레이어/블록 분석) | ✅         | → PDF               |
| **DXF** | AutoCAD 교환 포맷                            | ✅ QCAD `dwg2pdf`     | ✅ ODA SDK                               | ✅         | → PDF               |
...
> **2D 변환 전략**: 모든 2D 포맷 → PDF로 변환 후 S3 캐싱. 프론트는 PDF.js 하나로 통일.

### 2D 변환 도구: QCAD Pro Server

- **GitHub**: qcad/qcad ⭐ 3.8K
- **비용**: €478 영구 라이선스 (서버당), 갱신 시 30% 할인 (~€335/년), 안 해도 기존 버전 영구 사용
- **CLI**: `dwg2pdf`, `dwg2svg`, `dwg2bmp` 포함
- **DWG 엔진**: 내부적으로 ODA(Teigha) 라이브러리 사용
- **SaaS 사용**: 서버 라이선스로 웹 서비스에서 사용 허용. QCAD 앱 자체를 유저에게 배포/다운로드는 불가
- **배포**: Docker 컨테이너에 설치 (Qt + ODA 의존성 포함)
- **MVP에서 유일한 현실적 선택**: DWG→PDF CLI 변환이 가능한 도구 중 가격/품질 대비 대안 없음
...
| **SolidWorks** (.sldprt/.sldasm) | 네이티브 CAD | 기계설계 점유율 1위. 중소~중견 제조업          | ❌          | ❌               | ✅ CAD Exchanger 또는 ODA MCAD |
...
> **변환 전략**: 도면 원본 → S3 보관 (다운로드용) + GLB/PDF 캐시 생성 (뷰잉용). 프론트는 PDF.js + Three.js GLTFLoader 두 개만 사용. 사용자가 다운로드하면 원본(STEP, DWG 등)이 나옴.
...
채택 현황: Google (Android 3D), Meta, Apple (Quick Look), Shopify (상품 3D), Three.js, Babylon.js, Unity, Unreal 전부 GLB 네이티브 지원.

|             | 2D                  | 3D                                |
| ----------- | ------------------- | --------------------------------- |
| 원본        | DWG, DXF, DWF ...   | STEP, IGES, STL, OBJ ...          |
...
| -------------- | -------------------- | ---------------------------------------------- | ----------------------------- |
| **MVP**        | QCAD CLI (`dwg2pdf`) | MayoConv AppImage (`--input stp --output glb`) | PDF.js + Three.js             |
| **Growth**     | ODA SDK (QCAD 대체)  | ODA SDK (Mayo 대체)                            | PDF.js + Three.js (변경 없음) |
| **Enterprise** | ODA SDK (동일)       | ODA + CAD Exchanger (네이티브 추가)            | PDF.js + Three.js (변경 없음) |

> Growth 전환 시 ODA 하나가 QCAD + Mayo 둘 다 대체. 프론트 코드 수정 없이 백엔드 변환 엔진만 교체.
...
│  ├── ProcessBuilder → QCAD dwg2pdf (2D → PDF)
│  ├── ProcessBuilder → MayoConv AppImage (3D → GLB)
│  ├── 썸네일: QCAD dwg2bmp (2D), MayoConv --output png (3D)
...
# 2D: QCAD Pro Server (€478 라이선스 구매 후 설치)
RUN apt-get update && apt-get install -y libqt5core5a libqt5gui5 libqt5widgets5 libqt5svg5
COPY qcad/ /opt/qcad/
...
```java
// 2D: DWG → PDF
new ProcessBuilder("/opt/qcad/dwg2pdf", "input.dwg", "output.pdf")

// 2D 썸네일: DWG → BMP
new ProcessBuilder("/opt/qcad/dwg2bmp", "input.dwg", "thumbnail.bmp")

// 3D: STEP → GLB
new ProcessBuilder("/opt/mayo/mayoconv",
...
파일 업로드
  → S3 원본 저장
  → @Async 변환 Job
      ├── 2D (DWG/DXF) → QCAD dwg2pdf → PDF + dwg2bmp → PNG 썸네일
      └── 3D (STEP/IGES/STL/OBJ) → MayoConv → GLB + PNG 썸네일
