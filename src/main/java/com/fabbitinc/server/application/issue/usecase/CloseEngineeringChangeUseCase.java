...
public class CloseEngineeringChangeUseCase {
    ...
    public void execute(CloseEngineeringChangeCommand command) {
        AuthContext auth = currentAuthProvider.getCurrentAuth();
        EngineeringChange engineeringChange =
                engineeringChangeService.getEngineeringChangeByNumberOrThrow(command.issueNumber());
        engineeringChangeService.closeEngineeringChange(auth.userId(), engineeringChange);
        partRevisionWorkflowApi.closeEngineeringChange(auth.userId(), engineeringChange.getId());
    }

    public record CloseEngineeringChangeCommand(int issueNumber) {
    }
}
