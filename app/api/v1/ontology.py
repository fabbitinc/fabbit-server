...
def mapping_confirm(req: MappingConfirmRequest):
    """매핑 확정 → DB 저장 (Human-in-the-loop)"""
    mapping_json = req.mapping.model_dump()

    result = repo.save_mapping(req.org_id, req.name, req.original_headers, mapping_json)
    if not result:
...
def list_mappings(org_id: str):
    """저장된 매핑 목록 조회"""
    rows = repo.list_mappings(org_id)
    return [
        {
            "id": str(r["id"]),
            "org_id": r["org_id"],
...
async def ingest(
    ...
    row = repo.get_mapping(mapping_id, org_id)
    if not row:
    ...
    try:
        stats = service.ingest_dataframe(df, mapping, org_id)
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"인제스션 실패: {e}")

    repo.increment_mapping_usage(mapping_id)
    return stats
...
def query(req: QueryRequest):
    """자연어 질의 (테넌트 격리)"""
    try:
        return service.natural_language_query(req.question, req.org_id)
...
def query_cypher(req: CypherRequest):
    """자연어 → Cypher 변환 및 실행 (테넌트 격리 없음)"""
    try:
        cypher = service.text_to_cypher(req.question)
        results = service.execute_cypher_query(cypher)
