...
from app.modules.part.schemas import PartDefaultOwnerItem, PartDefaultOwnerListResponse
...
def list_default_owners(
    ...
    """기본 담당자/팀 설정 목록 조회."""
    rows = part_repo.list_default_owners(db)
    items = [_to_item(r, db) for r in rows]
    return PartDefaultOwnerListResponse(items=items)
